...
//   assign led = 8'hFF;
   ...
   defparam reset_shift_reg.INIT = 16'hFFFF;
	
   rs232send sendx(.clk(clock_27mhz), .rst(power_on_reset), .data(switch[7:0]), .start_send(start_send), 
	            .xmit_data(rs232_txd), .xmit_clk(xmit_clk), .led_debug(led[0]));
					
	assign led[3] = ~enter_db;
	assign led[2] = ~enter_old;
	assign led[1] = ~start_send;
	assign {led[7], led[6], led[5], led[4]} = 4'b1111;      
...
module rs232send(
    ...
    output reg xmit_clk,     // baud rate; sent to logic analyzer for debuggin
	 ...
	 // this module takes in data and shifts it out once start send is asserted high
    // start bit is a 0
    ...
    parameter STATE_IDLE      = 0;
	 ...
	 parameter BITS_PER_PACKET = 9;
	 
	 reg         state;
	 reg         start_send_saved;
    reg [15:0] count;
